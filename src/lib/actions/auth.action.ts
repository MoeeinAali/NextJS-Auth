...
import {JWT, UserResponse, UserSession} from "@/lib/types/auth.type";
import {jwtDecode} from "jwt-decode";
import {decryptSession, encryptSession} from "@/lib/utils/session.utils";
...
export async function signOutAction() {
    ...
    try {
        const response = await fetch(
            `${process.env.NEXT_PUBLIC_API}/api/identity/signout`,
            {
                method: "POST",
